...
function CustomMail({ mailId, onClose }) {
  ...
  const handleSendReply = async () => {
    ...
    try {
      await axios.post(
        `https://hiring.reachinbox.xyz/api/v1/onebox/reply/${mailId}`,
        {
          to: replyData.to,
          from: replyData.from,
  ...
  return (
    <div className="bg-gray-400/25 fixed top-0 left-0 flex justify-center items-center h-full w-full z-20">
      <div className="bg-[#141517] w-1/2 h-4/5 rounded-lg border border-[#41464B]">
        ...
        <div className="flex items-center h-18 ml-5 space-x-8">
          <div
            className="bg-gradient-to-r from-[#4B63DD] to-[#0524BFFC] px-2 rounded-md flex items-center cursor-pointer"
