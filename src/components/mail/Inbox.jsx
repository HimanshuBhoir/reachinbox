    ...
    <div className="dark:bg-[#141517] bg-white border dark:border-[#343A40] mx-8 rounded-md my-3">
      <div className="p-4">
        <div className="flex justify-between py-4">
          ...
          <div className="text-sm dark:text-[#7F7F7F] text-[#637381]">20 June 2022 : 9:16 AM</div>
        </div>
        <div
...
const Inbox = ({ selectMailId }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `https://hiring.reachinbox.xyz/api/v1/onebox/messages/${selectMailId}`,
        {
    ...
    <div className="overflow-y-scroll no-scrollbar h-full">
      ...
      <div className="py-8 mx-8 relative flex justify-center items-center">
        ...
        <div className="absolute inset-0 flex justify-center items-center">
          <div className="dark:bg-[#171819] bg-[#E0E0E0] px-4 py-1 text-sm text-black dark:text-white">Today</div>
        </div>
      </div>
      ...
      <div className="mx-8">
        {showPopUp && <CustomMail mailId={selectMailId} onClose={() => setShowPopUp(false)} />}
      </div>
      <div
      ...
      {showDelete && (
        <Delete onCancel={() => setShowDelete(false)} onDelete={handleDelete} />
      )}
    </div>
  );
};
